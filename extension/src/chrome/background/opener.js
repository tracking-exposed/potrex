...
bo.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === 'opener') {
    ...
    const vinfo = request.payload
    const buildPromise = _.partial(waitOpenCloseTab, vinfo.tabtime)
    const promiseList = _.map(vinfo.list, buildPromise)
